...
recordsconst::recordsconst(unsigned PTRsymb, unsigned char LENGTH,
  unsigned int BEGIN)
{
  ident = issymbol; ptrsymb = PTRsymb; length = LENGTH; begin = BEGIN;
}

recordstring::recordstring(unsigned PTRsymb, unsigned char LENGTH,
  unsigned int BEGIN)
{
  ident = isstring; ptrsymb = PTRsymb; length = LENGTH; begin = BEGIN;
}
...
*/
recordvar::recordvar(unsigned PTRsymb, unsigned char LENGTH,
  unsigned int  NUM)
{
  ident = isvar; ptrsymb = PTRsymb; length = LENGTH; num = NUM;
}

recordinteger::recordinteger(int VALUE)
{
  ident = isinteger; value = VALUE;
}

recordfloat::recordfloat(float VALUE)
{
  ident = isfloat; value = VALUE;
}


recordunknown::recordunknown() { ident = isunknown; }

recordcut::recordcut()
{
  ident = iscut; func = hpcut;
}
...
recordexpression::recordexpression(unsigned int LENGTH, unsigned int PRECORD)//(unsigned int *PTR,unsigned int LENGTH)
{
  ident = isexpression; length = LENGTH; precord = PRECORD;
}//ptr=PTR;length=LENGTH;}

recordfunction::recordfunction(unsigned char NARG, unsigned int FUNC, unsigned int PTRARG)//,unsigned int *PTRARG)
{
  ident = isfunction; narg = NARG; func = FUNC; ptrarg = PTRARG;
}//;ptrarg=PTRARG;}

recordlist::recordlist(unsigned int HEAD, unsigned int LINK)
{
  ident = islist; head = HEAD; link = LINK;
}

recordclause::recordclause(unsigned char IDENT, unsigned int NEXT,
  unsigned int NVARS, unsigned int HEAD, unsigned int PTRTARGET)//,unsigned int *PTRTARGET)
{
  ident = IDENT; next = NEXT; nvars = NVARS; head = HEAD; ptrtarget = PTRTARGET;
}//ptrtarget=PTRTARGET;}
...
void PrintSconst(char** Buf, size_t* BufSize, recordsconst* rsc,
  TScVar* ScVar, array* heap);
void PrintVar(char** Buf, size_t* BufSize, recordvar* pbr,
  TScVar* ScVar, array* heap);
void PrintInteger(char** Buf, size_t* BufSize, recordinteger* pri,
  TScVar* ScVar, array* heap);
void PrintUnknown(char** Buf, size_t* BufSize, recordunknown* pru,
  TScVar* ScVar, array* heap);
void PrintList(char** Buf, size_t* BufSize, recordlist* pru,
  TScVar* ScVar, array* heap);
void PrintExpression(char** Buf, size_t* BufSize,
  recordexpression* pbr, TScVar* ScVar, array* heap);
void PrintEmptyList(char** Buf, size_t* BufSize, recordemptylist* pr,
  TScVar* ScVar, array* heap);

void PrintFunction(char** Buf, size_t* BufSize,
  recordfunction* prf, TScVar* ScVar, array* heap)
{
  char pBuf[255];
  char* p = *Buf;
  ...
  {
    char* p = *Buf;
    PrintSconst(Buf, BufSize, (recordsconst*)pbr,
      ScVar, heap);
    //*Buf = 0;
  ...
  if (prf->narg && (*BufSize) > 2)
  {
    ...
    for (int i = 0; i < static_cast<int>(prf->narg); i++)
    {
      ...
      case issymbol:
        ...
        break;
      case isvar:
        PrintVar(Buf, BufSize, (recordvar*)pbr,
          ScVar, heap);
        break;
      case isinteger:
        PrintInteger(Buf, BufSize, (recordinteger*)pbr,
          ScVar, heap);
        break;
      case isunknown:
        PrintUnknown(Buf, BufSize, (recordunknown*)pbr,
          ScVar, heap);
        break;
      case islist:
        PrintList(Buf, BufSize, (recordlist*)pbr,
          ScVar, heap);
        break;
      case isexpression:
        PrintExpression(Buf, BufSize,
          (recordexpression*)pbr, ScVar, heap);
        break;
      case isemptylist:
        PrintEmptyList(Buf, BufSize,
          (recordemptylist*)pbr, ScVar, heap);
        break;
      default:
        int len = *Buf - p;
        if (len > 0)
...
void PrintExpression(char** Buf, size_t* BufSize,
  recordexpression* pex, TScVar* ScVar, array* heap)
{
  if (2 > *BufSize)
  {
    out(const_cast<char*>("Buffer for unknown out is too small"));
...
void PrintList(char** Buf, size_t* BufSize, recordlist* rl,
  TScVar* ScVar, array* heap)
{
  char pBuf[255];
  char* p = *Buf;
...
void PrintVar(char** Buf, size_t* BufSize, recordvar* prv,
  TScVar* ScVar, array* heap)
{
...
void PrintClause(recordclause* rc, TScVar* ScVar, array* heap)
{
  ...
  for (int i = 0; *(ptarget + i); i++)
  {
    ...
    {
    case isfunction:
      PrintFunction(&p, &BufSize, rf, ScVar, heap);
      break;
    case issymbol:
      PrintSconst(&p, &BufSize, (recordsconst*)rf,
        ScVar, heap);
      break;
    default:
      sprintf(_Buf, "PrintClause: unknown ident: %d", rf->ident);
    }
  }
  if (BufSize > 2)
    sprintf(p, ".");
...
void PrintProgram(TScVar* ScVar, array* heap)
{
  ...
  {
    //recordsconst * ptr = heap->GetPrecordsconst(*heap->GetPunsigned(tat[i]));
    ...
    strncpy(_Buf, name, Length); _Buf[Length] = 0;
    out(_Buf);
    if (ptr->begin)
    {
