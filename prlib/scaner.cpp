...
{	//функция синтаксического разбора строки
	...
	while(*p && !err)    //возвратит 0 или код ошибки в конструкции
	{
		switch(*p)
		{
			case ',' :buf[bptr++] = iscomma;
				break; //запись запятой в buf
			case '|' :buf[bptr++] = isstick;
				break;
			case '(' :
			{
				if (!exprip)
			...
			case '.' :
			{
				...
				right = false; bptr = gptr = exprip = novar = 0;
				if (Query)
				{	if (*p) p++; 
					return err;
			...
			case ']' : buf[bptr++] = islend;
				err = list(ScVar, heap);
				break;
			case '\"': err = strings(p, ScVar, heap);
				break;//строка символов
			case '?' :	//вопрос
			{	if (bptr || right) 
					err = 10;
				else 
			...
			case '\r':
			case ' ' :
				break;//ни каких действий - переход на следующий символ
			case '_' ://анонимка
			{
				if (gptr <= 0) 
					err = 11;
			...
			case '*' :	err = arsgn(ScVar, ismult);
				break;
			case '/' :	err = arsgn(ScVar, isslash);
				break;
		...
		}	if (*p) p++; //при выходе из любой процедуры lptr должен указывать
