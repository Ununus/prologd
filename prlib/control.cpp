...
unsigned menegerbp(unsigned name, TScVar* ScVar,
...
int expand_stack(TClVar* ClVar)
{
  ...
  */
  ...
  delete[] ClVar->st_con; ClVar->st_con = st_conN;
  delete[] ClVar->st_vr1; ClVar->st_vr1 = st_vr1N;
  delete[] ClVar->st_vr2; ClVar->st_vr2 = st_vr2N;
  delete[] ClVar->st_trail; ClVar->st_trail = st_trailN;
...
void to_control(TClVar* ClVar, array* heap) //запись в стек
{
  ...
  if (ClVar->scptr + 7 > ClVar->vmaxstack &&
    expand_stack(ClVar) != 0)
  {
    outerror(28);
...
unsigned occur_term(unsigned* TERM, unsigned* FRAME,
  TClVar* ClVar, array* heap)
{
  int argt = -1;
  unsigned j;
  ...
  do
  {
    ...
    case isexpression:
    {   unsigned char ind =
      calculation(*TERM, *FRAME, &i, &f, ClVar, heap);
    switch (ind)
    {
    case isinteger:
    {   recordinteger pi(i);
...
bool nextarg(unsigned* j, TScVar* ScVar, TClVar* ClVar,
  array* heap)//j для печати разделителя
{
  bool nxaex;
  do
  {
    ...
    case islist:
      ...
      else
      {
        ...
        switch (a)
        {
        case 3: buff[*j] = ',';   (*j)++;   break;
        case 2: buff[*j] = ']';   (*j)++;   break;
        default:buff[*j] = '|'; (*j)++;
          //term=trm;
          nxaex = stac(ClVar, heap);
...
unsigned write_term(unsigned TERM, unsigned FRAME, unsigned W,
  unsigned j, TScVar* ScVar, TClVar* ClVar, array* heap)//,FILE *d=NULL)
{
  unsigned i = 0;
  ...
  arg = 0;
  while (ex)
  { //здесь цикл8ический вывод терма
    ...
    else
    {
      ...
      case isfunction:
      {
        ...
        for (i = 0; i < (unsigned)ClVar->precordsconst->length;
          buff[j++] = *(name + i), i++);
        buff[j++] = '(';
        arg = 2;
        flaglist = false;
      ...
      for (i = 0; j < sizeof(buff) - 1 && string[i];
        buff[j++] = string[i++]);
      buff[j] = 0; i = 0;
      ...
      for (i = 0; i < (unsigned)ClVar->precordsconst->length;
        buff[j + i] = *(name + i), i++);
      j += i; i = 0; ex = nextarg(&j, ScVar, ClVar, heap);
      }   break;//ok
      case islist:
      ...
      case isexpression:
        ...
        float ff;
        unsigned char ind =
          calculation(term, frame, &ii, &ff, ClVar, heap);
        switch (ind)
        {
        case isinteger:
        {
...
char yes[] = "ДА";
char no[] = "НЕТ";
void prvars(TScVar* ScVar, TClVar* ClVar, array* heap)    //пока только описание функции
{
  if (!ClVar->varqu)
  {
    out(yes);
  ...
  else
  {
    ...
    for (unsigned k = 0; k < ClVar->varqu; k++)
    {
      ...
      for (j = 0; j < (unsigned)ClVar->precordvar->length;
        buff[j] = *(name + j), j++);
      buff[j++] = '=';
...
{
  if (ClVar->newclause && ClVar->newclause != isnil)
  {   //pnclause=head->at(newclause);pncltarget=head->at(newclause-1);
    ...
    if (ClVar->svptr > ClVar->vmaxstack &&
      expand_stack(ClVar) != 0)
    {
      ClVar->err = 28;
      ClVar->stat = 1;
    ...
    else
    {
      ...
      if (unify(heap->ptcltarget[ClVar->tryclause],
        ClVar->head, ClVar->frame1, ClVar->frame2, ClVar, heap))
      {   //удачная унификация
...
int control(TScVar* ScVar, TClVar* ClVar, array* heap, bool* EnableRunning)
{
  ...
  while (ClVar->stat != 1 && !ClVar->err)
  {
    ...
    case 6:
      ...
      {
      case isfunction:
        ClVar->bipr = ClVar->precordfunction->func;
        break;
      case iscut:
      {   recordcut* pcut =
        (recordcut*)ClVar->precordfunction;
      ClVar->bipr = pcut->func;
      }   break;

      default:
        ClVar->bipr = ClVar->head;
      }
...
// ================ унификация =======================
unsigned char calculation(unsigned term, unsigned frame,
  long* i, float* f, TClVar* ClVar, array* heap)
...
// array st(maxstaccalc);
  ...
  {//if (lowMemory()) return 0;//!!!сообщение о r_t_e
    ...
    if (ptr[j] < isbase)//операнд
    {
      ...
      case isinteger:
      {
        ClVar->precordinteger = (recordinteger*)tp;
        st[index++] =
          new recordinteger(ClVar->precordinteger->value);
      ...
      case isfloat:
      {
        ClVar->precordfloat = (recordfloat*)tp;
        st[index++] =
          new recordfloat(ClVar->precordfloat->value);
      ...
      default:
        err = 32;//других типов не должно быть
      }
    }
    else //знак ариафм операции
    {
      ...
      st[index++] = (ind == isinteger) ?
        (void*)new recordinteger(oi2) :
        (void*)new recordfloat(of2);
      if (!st[index - 1])
        err = 2;
    }
  }
...
bool unfun(TClVar* ClVar, array* heap)
{
  ...
  case isfunction:
    recordfunction
      * pf1 = heap->GetPrecordfunction(ClVar->term1),
      //(recordfunction *)&heap->heaps[ClVar->term1],
      * pf2 = (recordfunction*)tp;
    if ((pf1->narg == pf2->narg) &&
      (pf1->func == pf2->func))
    {
      ClVar->numb = -1;
      ret = stac_un(ClVar);
...
bool unsymb(TClVar* ClVar, array* heap)
{
  ...
  case issymbol:
  {	recordstring
    * pstr1 = heap->GetPrecordstring(ClVar->term1),
  ...
  if (pstr1->length == pstr2->length &&
    !strncmp(name1, name2, pstr1->length))
    /*if (term1==term2)*/
  {   /*bf[bp-5]++;*/nextun(ClVar, heap);
  }
  else
    ret = false;
  }     break;
  ...
  case isexpression:
    ret = false;	break;
  case isunknown:
    nextun(ClVar, heap); break;
  }
  return ret;
}

void ocrn(unsigned bd1, unsigned tr2, TClVar* ClVar)//установка связи в стеке переменных
{
...
bool unvar(TClVar* ClVar, array* heap)
{
  ...
  else
  {
    ...
    case isexpression:
      ...
      float af;
      unsigned char ind =
        calculation(ClVar->term2, ClVar->frame2, &a, &af,
          ClVar, heap);
      switch (ind)
      {
      case 0:
        ret = false; break;//!!!abort_calculation;
      case isinteger:
      {	recordinteger ptr(a);
...
bool unint(TClVar* ClVar, array* heap)
{
  ...
  //(recordinteger *)&heap->heaps[ClVar->term1];
  if (precordfloat->value - 1 < precordinteger->value &&
    precordfloat->value == (float)precordinteger->value)
  {
    ret = true;
    nextun(ClVar, heap);
  }
  ...
  {	float af;
  long a;
  unsigned char ind = calculation(ClVar->term2,
    ClVar->frame2, &a, &af, ClVar, heap);
  recordinteger* pint = heap->GetPrecordinteger(ClVar->term1);
  //(recordinteger *)&heap->heaps[ClVar->term1];
...
bool unfloat(TClVar* ClVar, array* heap)
{
  ...
  recordinteger* precordinteger =
    (recordinteger*)tp;
  if (precordfloat->value - 1 < precordinteger->value &&
    (float)precordinteger->value == precordfloat->value)
  {
    ret = true;
    nextun(ClVar, heap);
  }
...
bool unexpr(TClVar* ClVar, array* heap)
{
  ...
  case isexpression:
    ...
    float cf, df;
    unsigned char
      ind1 = calculation(ClVar->term1, ClVar->frame1, &c, &cf, ClVar, heap),
      ind2 = calculation(ClVar->term2, ClVar->frame2, &d, &df, ClVar, heap);
    if (ind1 == ind2 && ind1 == isinteger)  //!!! сделано только для целых
      ...
      else;// ret=false; уже присвоено
    else;// ret =false;//!!! нужно сообщение об ошибке периода исполнения ret=false уже
  }
  return ret;
}
  ...
  TClVar* ClVar, array* heap)
{
  ...
  ClVar->fr2 = ff2;
  while (ClVar->ex && ret)
  {//!!!проверрить на прерывание программы
  ...
  //=========
    ...
    {
    case isfunction:
      ret = unfun(ClVar, heap);
      break;
    case isstring:
    case issymbol:
      ret = unsymb(ClVar, heap);
      break;
    case isvar:
      ret = unvar(ClVar, heap);
      break;
    case isinteger:
      ret = unint(ClVar, heap);
      break;
    case isfloat:
      ret = unfloat(ClVar, heap);
      break;
    case islist:
      ret = unlist(ClVar, heap);
      break;
    case isunknown:
      nextun(ClVar, heap);
      ret = true;
      break;
    case isemptylist:
      ret = unempty(ClVar, heap);
      break;
    case isexpression:
      ret = unexpr(ClVar, heap);
      break;  //ok
    default:
      return false;
    }
