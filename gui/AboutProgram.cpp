...
void Help::setCurosrIfFound(QString word)
{
  ...
  bool fnd = false;
  if (!word.isEmpty()) {
    fnd = m_help_text->find(QString::fromLocal8Bit("Âñòðîåííûé ïðåäèêàò ") + word, QTextDocument::FindWholeWords);
  }
  if (fnd) {
    // Ïåðåäâèíóòü íàéäåííîå íàâåðõ.
    // Ïî÷åìó òî ñêðîëë íå ðàáîòàåò, äâèãàþ êóðñîðîì
