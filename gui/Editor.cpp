...
static bool canBeFunction (QString& str, int start, int &end) {
  ...
  for (int i = start; i < str.size(); ++i) {
    ...
    if (op == 0 && !othop && (str[i] == ':' || str[i] == '.' || str[i] == ',' ||str[i] == ';')) return true;
    if (str[i] == "\"" && (i == 0 || (i > 1 && str[i - 1] != '\\'))) othop = !othop;
    if (othop) continue;
    if (str[i] == '(') {
      ++op;
    }
...
{
  if (e->modifiers().testFlag(Qt::KeyboardModifier::ShiftModifier) &&
...
void Editor::keyPressEvent(QKeyEvent *keyEvent)
{
  ...
  if (!isShortcut && (hasModifier || keyEvent->text().isEmpty()|| completionPrefix.length() < 1
...
void Editor::keyReleaseEvent(QKeyEvent *e) {
  if (e->key() == Qt::Key_Control) m_ctrl_pressed = false;
