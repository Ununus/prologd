...
struct recordsconst : public baserecord //строковая константа
{
  ...
  recordsconst(unsigned PTRsymb, unsigned char LENGTH,
...
struct recordstring :public baserecord
{
  ...
  recordstring(unsigned PTRsymb, unsigned char LENGTH,
    unsigned int BEGIN = 0);
...
struct recordvar : public baserecord    //структура записи переменной
{
  ...
  recordvar(unsigned PTRsymb, unsigned char LENGTH,
    unsigned int  NUM = 0);
};
...
// unsigned int *ptrarg; //указатель на массив индексов в массиве heap
 ...
 //индекс массива в на 1 меньше чем индекс функции
  recordfunction(unsigned char NARG, unsigned int FUNC,
    unsigned int PTRARG);
};
...
//предложения
  recordclause(unsigned char IDENT, unsigned int NEXT,
    unsigned int NVARS, unsigned int HEAD, unsigned int PTRTARGET);
};
//-----------------------------------------------
...
struct TClVar
{
  ...
  unsigned int
    newclause,  //индекс в массиве указывающий на предложения
